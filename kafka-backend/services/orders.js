...
async function handle_request(msg, callback) {
    ...
    switch (msg.api) {
        ...
        case "GET_ORDER_BY_CUSTOMER":
            {
                Orders.find({ _id: msg.body.customer_id }, (err, result) => {
                    if (err) {
        ...
        case "GET_ORDER_BY_ID":
            {
                Orders.findOne({ _id: msg.body.order_id }).populate('cart_items'
                ).populate('delivery_address').populate({ path: 'cart_items.dishes', model: 'Dish' }).exec((err, result) => {
                    if (err) {
                        console.log('Error occured while fetching Order details' + err)
                        callback(err, 'Error')
                    }
                    else {
                        console.log('Order details fetched' + result.restaurant_id)
